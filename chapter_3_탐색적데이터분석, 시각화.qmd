...
1. gdp.csv 파일을 불러오려고 해 파이썬 코드를 제시해줘
2. unemployment.csv 파일을 첫 3행은 제외하고 불러오려고 해 파이썬 코드를 제시해줘
3. domestic.xlsx 파일의 KOR 시트를 첫 5행은 제외하고 불러오려고 해 파이썬 코드를 제시해줘
4. auto.dta 파일 불러오려고 해 파이썬 코드를 제시해줘
:::
<br>
...
df = pd.read_csv('data/chapter_3/gdp.csv')
```
<br>
...
```{python}
## 경로를 변수에 입력
csv_file_path = 'data/chapter_3/gdp.csv'
...
df_99999 = pd.read_csv('data/chapter_3/gdp_99999.csv')
df_99999.head(3)
```
<br>
...
::: {.callout-tip title=Matplotlib? Seaborn?"}
**Seaborn**은 고급 통계적 시각화에 중점을 둔 라이브러리로, 데이터 분석을 더 쉽게 만들어주는 기본 테마와 다양한 플롯 유형을 제공합니다.  
**Matplotlib**은 저수준의 세밀한 제어가 가능한 범용 시각화 라이브러리로, 맞춤화와 유연성이 뛰어납니다.  
결론적으로, 빠른 분석에는 Seaborn, 복잡한 커스터마이징에는 Matplotlib을 사용하는 것이 일반적입니다.
:::
...
# 작업 폴더 내 NanumGothic.ttf 폰트를 등록하고 설정
font_path = 'data_book/fonts/NanumGothic.ttf'
fontprop = fm.FontProperties(fname=font_path)
...
data = pd.read_csv('data/chapter_3/gdp.csv')
...
![그림판](data_book/images/chapter_3/Graph.png){#fig-graph1 width=7.5%}

![STATA](data_book/images/chapter_3/histogram.png){#fig-graph2 width=7.5%}
